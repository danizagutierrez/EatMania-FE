...
export const authSlicer = createSlice({
    ...
    reducers: {
        ...
        loginUser: (state, action: any) => {
            const { userEmail, password, firstName, lastName } = action.payload.data;
          
            const foundUser = state.userList.find((user) => user.user_email === userEmail);
          
            if (foundUser) {
              if (foundUser.user_password === password) {
                // Valid credentials, update user info
                state.user = foundUser;
                state.isAuthenticated = true;
                state.errors = { password: false, email: false };
              } else {
                // Invalid password
              ...
              // User not found, set new user info
              const newUser = {
                user_id: state.userList.length + 1, // You may need a more sophisticated ID generation logic
